...
function updateStats() {
  if (statBest) {
    statBest.textContent = Math.max(
      ...window.habits.map((h) => h.best),
      0
    );
  }

  if (statStreak) {
    statStreak.textContent = Math.max(
      ...window.habits.map((h) => h.streak),
      0
    );
  }

  if (statTotal) {
    statTotal.textContent = window.habits.reduce(
      (s, h) => s + h.total,
      0
    );
  }
...
function updateProgress() {
  ...
  const done = window.habits.filter(
    (h) => h.completedToday
  ).length;
...
function refreshChips(card, h) {
  ...
  const streakValue =
    card.querySelector(".chip-streak .chip-value");

  const totalValue =
    card.querySelector(".chip-total .chip-value");
